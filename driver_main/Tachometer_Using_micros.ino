...
Made changes to Raw Frequency (FrequencyRaw) calculation to handle for IntegerDivideByZero errors.
...
const unsigned long ZeroTimeout = 500000;  // For high response time, a good value would be 100000.
...
void setup()  // Start of setup:
{
  ...
  attachInterrupt(2, Pulse_Event, FALLING);  // Enable interruption pin 2 when going from LOW to HIGH.
...
void loop()  // Start of loop:
{
  ...
  if (PeriodAverage != 0) {
    ...
    FrequencyRaw = 0;  // Default to zero if PeriodAverage is zero to avoid divide-by-zero exception.
  }
